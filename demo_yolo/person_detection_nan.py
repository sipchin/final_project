...
class ObjectDetection(object):
            ...
            for i in range(len(boxes)):
                if i in indexes:
                    ...
                    if label == "person":
                        ...
                        #self.pubx(c_x)
                        #self.puby(c_y)
                        center = Float32MultiArray(data=[x+w/2,y+w/2])
                        print(type(center))
                        self.pub_array(center)
                        print(f"x = {x} , y = {y} , x+w = {x+w} , y+h ={y+h}")
                        ...
                        if key == 27:
                         break
    def pubx(self,x):
        pub = rospy.Publisher('center', Float32, queue_size=10)
    ...
    def pub_array(self,arr):
        pub = rospy.Publisher('center', Float32MultiArray, queue_size=10)
        #rospy.init_node('talker1', anonymous=True)
        rate = rospy.Rate(10)
        #while not rospy.is_shutdown():
        #hello_str = x
